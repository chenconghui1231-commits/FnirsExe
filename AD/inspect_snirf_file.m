function inspect_snirf_file(filepath)
    % 检查SNIRF文件结构
    fprintf('=== SNIRF文件结构分析 ===\n');
    fprintf('文件: %s\n', filepath);
    ...
    % 显示主要数据集
    fprintf('\n=== 主要数据集 ===\n');
    list_datasets(file_info, '');
    
    % 读取关键数据
    fprintf('\n=== 关键数据内容 ===\n');
    read_key_data(filepath);
end
...
function list_datasets(info, current_path)
    % 递归列出所有数据集
    for i = 1:length(info.Groups)
        group = info.Groups(i);
        full_path = [current_path '/' group.Name];
        fprintf('组: %s\n', full_path);
        list_datasets(group, full_path);
    end
...
function read_key_data(filepath)
    try
        % 1. 读取波长信息
        ...
        if h5exists(filepath, '/nirs/data1/dataTimeSeries')
            ...
            % 读取前几行数据预览
            if data_size(1) > 0
                preview_data = h5read(filepath, '/nirs/data1/dataTimeSeries', [1 1], [min(5, data_size(1)) data_size(2)]);
                fprintf('数据预览 (前5行):\n');
                disp(preview_data);
            end
        else
        ...
        % 4. 读取测量列表信息
        fprintf('\n=== 测量列表信息 ===\n');
        read_measurement_lists(filepath);
...
function read_measurement_lists(filepath)
    % 读取所有measurementList条目
    ml_count = 0;
    i = 1;
